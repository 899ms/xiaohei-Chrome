  ...
  const server = net.createServer((client) => {
    ...
    const receiveHeader = (chunk) => {
      ...
      (async () => {
        ...
        if (process.env.BROWSEROPS_PROXY_DIAGNOSTICS === '1') notify('REQUEST', target.host + ':' + target.port);
        ...
        if (process.env.BROWSEROPS_PROXY_DIAGNOSTICS === '1') notify('CONNECTED', target.host + ':' + target.port);
...
async function lookupProxyCountry(config) {
  const response = await requestProxyHttp(config, 'ip-api.com', '/json/?fields=status,message,countryCode,query,timezone,lat,lon');
  ...
  const ip = String(value.query || ''); const countryCode = String(value.countryCode || '').toUpperCase();
  if (value.status !== 'success' || !ip || !/^[A-Z]{2}$/.test(countryCode)) throw new Error(String(value.message || 'Proxy exit lookup response was incomplete'));
  return { ip, countryCode, timezone: String(value.timezone || ''), latitude: Number.isFinite(Number(value.lat)) ? Number(value.lat) : null, longitude: Number.isFinite(Number(value.lon)) ? Number(value.lon) : null, checkedAt: new Date().toISOString() };
}
...
module.exports = { parseProxy, displayProxy, startAuthenticatedProxy, lookupProxyCountry, probeProxyTunnel, probeProxyHttps };
