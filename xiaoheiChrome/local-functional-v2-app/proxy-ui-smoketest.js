...
async function evaluate(expression, awaitPromise = false) {
  ...
  if (!tab) throw new Error('BrowserOps renderer is not available on port 9333');
...
async function main() {
  ...
  if (action === 'inspect') {
    const value = await evaluate(`(() => {
      const ui = JSON.parse(localStorage.getItem('browserops-v2-ui-state') || '{}');
  ...
  if (action === 'test') {
    const started = await evaluate(`(() => {
      const ui = JSON.parse(localStorage.getItem('browserops-v2-ui-state') || '{}'); const profile = (ui.profiles || []).find((item) => item.id === ${JSON.stringify(id)});
  ...
  if (action === 'apply-tested') {
    console.log(JSON.stringify(await evaluate(`(async () => {
      ...
      const ui = JSON.parse(localStorage.getItem('browserops-v2-ui-state') || '{}'); const profile = (ui.profiles || []).find((item) => item.id === ${JSON.stringify(id)}); if (!profile) return { status: 'missing' };
      profile.proxy = smoke.proxy; profile.exitIp = smoke.ip; profile.exitCountryCode = smoke.country; profile.exitCheckedAt = smoke.checkedAt; localStorage.setItem('browserops-v2-ui-state', JSON.stringify(ui)); await window.ops.syncProfiles(ui.profiles);
  ...
  if (action === 'logs') { console.log(JSON.stringify(await evaluate(`(() => { const ui = JSON.parse(localStorage.getItem('browserops-v2-ui-state') || '{}'); return (ui.logs || []).slice(0, 12); })()`))); return; }
  ...
  if (action === 'bulk-test') {
    ...
    console.log(JSON.stringify(await evaluate(`(() => {
      const endpoints = ${JSON.stringify(endpoints)}; const ui = JSON.parse(localStorage.getItem('browserops-v2-ui-state') || '{}'); const profile = (ui.profiles || []).find((item) => item.id === ${JSON.stringify(id)});
  ...
  if (action === 'start-endpoint') {
    const host = process.argv[4]; const port = Number(process.argv[5]); if (!host || !Number.isInteger(port)) throw new Error('Host and port are required');
    console.log(JSON.stringify(await evaluate(`(() => {
      const ui = JSON.parse(localStorage.getItem('browserops-v2-ui-state') || '{}'); const source = (ui.profiles || []).find((item) => item.id === ${JSON.stringify(id)}); if (!source) return { status: 'missing' };
