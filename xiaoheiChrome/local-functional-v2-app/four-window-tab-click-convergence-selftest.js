    ...
    for (let step = 1; step <= 12; step += 1) {
      ...
      const result = await waitFor(async () => {
        ...
        for (let index = 0; index < sessions.length; index += 1) {
          ...
          const clicks = await evaluate(tab, `Number(localStorage.getItem('browserops-durable-clicks')||0)`); values.push({ url: tab.url, clicks });
        }
