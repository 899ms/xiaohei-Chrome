...
async function popupTargets(session) { return extensionTargets(session, 'popup.html'); }
async function sesTargets(session) { return extensionTargets(session, 'ses.html'); }

async function main() {
  const root = path.join(__dirname, '..', 'four-window-extension-popup-data');
  ...
  await fs.writeFile(path.join(extensionPath, 'manifest.json'), JSON.stringify({
    manifest_version: 3,
    name: 'BrowserOps Popup Sync Probe',
    version: '1.0.0',
    action: { default_popup: 'popup.html', default_title: 'Popup Sync Probe' },
    sandbox: { pages: ['ses.html'] },
  }, null, 2));
  await fs.writeFile(path.join(extensionPath, 'popup.html'), '<!doctype html><meta charset="utf-8"><style>body{width:300px;height:190px;margin:0;overflow:hidden}iframe{border:0;width:100%;height:100%}</style><iframe id="ui-ses-iframe" src="ses.html?sestheme=light"></iframe>');
  ...
  const profiles = Array.from({ length: 4 }, (_, index) => ({ id: 'popup-env-' + (index + 1), name: index ? ('Popup Slave ' + index) : 'Popup Master', browser: 'Google Chrome', proxy: 'Direct' }));
  const events = [];
  ...
  try {
    ...
    await wait(700);
    const driverOutput = await run(path.join(__dirname, 'native-extension-popup-driver.exe'), sessions.map((session) => String(session.pid)));
    ...
    assert(popupLists.every((list) => list.length === 1), 'extension popup did not converge across four environments: ' + popupLists.map((list) => list.length).join('/'));
    ...
    for (let attempt = 0; attempt < 30; attempt += 1) {
      focusedIds = (await readActive()).map((result) => String(result.result?.value || ''));
      if (focusedIds.every((value) => value === 'password')) break;
      await wait(100);
    }
    ...
    process.stdout.write(JSON.stringify({
      ...
      popupTargets: popupLists.map((list) => list[0].url),
