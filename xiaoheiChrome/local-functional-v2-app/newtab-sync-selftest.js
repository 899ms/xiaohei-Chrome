...
const deepScript = `(() => { const all=[]; const walk=(root)=>{for(const e of root.querySelectorAll('*')){if(e.matches('input,textarea'))all.push(e);if(e.shadowRoot)walk(e.shadowRoot);}};walk(document);const preferred=all.find(e=>e.type==='search')||all.find(e=>e.type==='text')||all[0];return {values:all.map(e=>({type:e.type,value:e.value,placeholder:e.placeholder})),preferred:preferred?.value||'',marker:document.getElementById('browserops-environment-marker')?.textContent||'',master:Boolean(document.getElementById('browserops-master-marker'))};})()`;
