...
const server = http.createServer((_request, response) => {
  ...
  response.end('<!doctype html><meta charset="utf-8"><title>BrowserOps V2 Selftest</title><input id="text" autofocus><h1>CDP Selftest</h1>');
});
...
async function main() {
  ...
  try {
    ...
    for (const item of started) {
      extensionMarkers[item.id] = await evaluate(item.port, "Boolean(document.getElementById('browserops-profile-marker'))");
