...
async function main() {
  ...
  try {
    ...
    const extensionRoot = path.join(process.env.APPDATA || '', 'browserops-local-sync', 'chrome-store-extensions');
    ...
    if (finalPages.length !== 1 || !/browserops-start\\.html/i.test(finalPages[0].url)) throw new Error('Startup extension page was not suppressed: ' + JSON.stringify(finalPages));
