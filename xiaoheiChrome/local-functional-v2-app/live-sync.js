...
const injection = String.raw`(() => {
  if (window.__browserOpsLiveSyncInstalled) return;
  window.__browserOpsLiveSyncInstalled = true;
  const send = (type, data) => { try { window.browserOpsSync(JSON.stringify({ type, ...data })); } catch (_) {} };
...
class LiveSyncController {
  ...
  async start(ids) {
    ...
    await this.connection.command('Runtime.addBinding', { name: 'browserOpsSync' });
