...
internal static class NativeInputMirror
{
    ...
    public static int Main(string[] args)
    {
        ...
        syncKeyboard = ReadBoolSetting("BROWSEROPS_SYNC_KEYBOARD", true); syncClick = ReadBoolSetting("BROWSEROPS_SYNC_CLICK", true);
        syncScroll = ReadBoolSetting("BROWSEROPS_SYNC_SCROLL", true); syncTrack = ReadBoolSetting("BROWSEROPS_SYNC_TRACK", true);
        delayClick = ReadBoolSetting("BROWSEROPS_DELAY_CLICK", false); delayInput = ReadBoolSetting("BROWSEROPS_DELAY_INPUT", false);
        inputMinMs = ReadIntSetting("BROWSEROPS_INPUT_MIN_MS", 300); inputMaxMs = Math.Max(inputMinMs, ReadIntSetting("BROWSEROPS_INPUT_MAX_MS", inputMinMs));
        clickMinMs = ReadIntSetting("BROWSEROPS_CLICK_MIN_MS", 100); clickMaxMs = Math.Max(clickMinMs, ReadIntSetting("BROWSEROPS_CLICK_MAX_MS", clickMinMs));
        ...
        Thread worker = new Thread(WorkerLoop) { IsBackground = true, Name = "BrowserOps input mirror" };
        worker.Start();
