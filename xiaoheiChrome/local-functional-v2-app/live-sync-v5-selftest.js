...
async function main() {
  ...
  try {
    ...
    const marker = { master: await evaluate(initialMasterTab, `Boolean(document.getElementById('browserops-master-marker'))`), slave: await evaluate(initialSlaveTab, `Boolean(document.getElementById('browserops-master-marker'))`) };
    ...
    if (resetTabs.length !== 1 || !resetTabs[0].url.toLowerCase().includes('browserops-start.html')) throw new Error('Restart tab reset failed: ' + JSON.stringify(resetTabs));
