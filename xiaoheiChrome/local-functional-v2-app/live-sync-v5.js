...
function isEnvironmentStartUrl(value) { return /browserops-start\.html/i.test(String(value || '')); }
function environmentStartUrl(engine, id) {
  const root = engine.running.get(id)?.root;
...
class LiveSyncController extends LiveSyncV4 {
  ...
  updateSettings(value = {}) {
    ...
    this.emit({ type: 'sync-settings', settings: { ...next } });
    return { ...next };
  }

  getSettings() { return { ...this.syncSettings }; }
  ...
  startNativeInputMirror() {
    ...
    if (process.platform !== 'win32' || !this.master) return;
    ...
    const nativeEnv = { ...process.env, BROWSEROPS_SYNC_KEYBOARD: this.syncSettings.keyboard ? '1' : '0', BROWSEROPS_SYNC_CLICK: this.syncSettings.click ? '1' : '0', BROWSEROPS_SYNC_SCROLL: this.syncSettings.scroll ? '1' : '0', BROWSEROPS_SYNC_TRACK: this.syncSettings.track ? '1' : '0', BROWSEROPS_DELAY_CLICK: this.syncSettings.delayClick ? '1' : '0', BROWSEROPS_DELAY_INPUT: this.syncSettings.delayInput ? '1' : '0', BROWSEROPS_INPUT_MIN_MS: String(this.syncSettings.inputMinMs), BROWSEROPS_INPUT_MAX_MS: String(this.syncSettings.inputMaxMs), BROWSEROPS_CLICK_MIN_MS: String(this.syncSettings.clickMinMs), BROWSEROPS_CLICK_MAX_MS: String(this.syncSettings.clickMaxMs) };
  ...
  async forward(tabId, payload) {
    ...
    return super.forward(tabId, payload);
  }
  ...
  async attachExtensionPage(tab) {
    ...
    try {
      await connection.command('Runtime.addBinding', { name: 'browserOpsSync' });
  ...
  async handleExtensionPage(tabId, event) {
    ...
    if (event.method === 'Runtime.bindingCalled' && event.params?.name === 'browserOpsSync') {
  ...
  async syncZoom(masterTabId, factor) {
    let corrected = 0;
    const masterTab = this.masterTabs.find((tab) => tab.id === masterTabId);
    const equivalentUrl = (a, b) => { const x=String(a||'').toLowerCase().replace(/\/$/,''); const y=String(b||'').toLowerCase().replace(/\/$/,''); const newTab=(v)=>v==='chrome://newtab'||v==='chrome://new-tab-page'; return x===y || (newTab(x)&&newTab(y)) || (x.includes('browserops-start.html')&&y.includes('browserops-start.html')); };
    await Promise.all(this.slaves.map(async (slave) => {
